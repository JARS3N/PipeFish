...
#
pH.index<-c(lapply(linedata,LowLine),lapply(X=.,linedata,HighLine))%>%Reduce(x=.,f='rbind')
...
lapply(function(x){data.frame(gain=diff(linedata[[x]][1:150])/diff(pHvals)/1000,pH=pHvals[-1],dye=x)}) %>% Reduce(f='rbind',x=.) %>% mutate(data=.,dye=factor(dye))
