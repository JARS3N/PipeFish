...
titrPlot<-ggplot(data,aes(x=pH,y=counts,colour=dye,fill=dye))+geom_jitter(aes(fill=dye),size=2,shape=16,alpha=.7) +geom_line(data=titrlines,aes(x=pH,y=val,colour=dye))+theme_few()+ggtitle('Titration Curve')+guides(colour = FALSE,fill=FALSE) +scale_colour_manual(values = cols)
